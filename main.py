...
def main():
    ...
    while True:
        ...
        if short_code == "ex":
            ...
            print(" ")
            print(
                f"New safe created for: {username}. Your password is: {password}. Keep it safe for you will need it for subsequent logins"
            ...
            if user_exists == username:
                ...
                while True:
                    ...
                    print("-" * 60)
                    print(
                        "Use the following codes for credentials management: \n cc-Create a Credential \n sc-Show Saved Credentials \n dc-Delete Credential \n ex-Exit"
                    )
                    short_code = input("Enter a choice: ").lower().strip()
                    print("-" * 60)
                    if short_code == "ex":
                        print(" ")
                        print(
                            f"See you soon {username}, meanwhile your secret is safe with us"
                        )
                        break
                    elif short_code == "cc":
                        print(" ")
                        ...
                        account_name = input(
                            "Enter your account's name - The username on this site "
                        ).strip()
                        while True:
                            print(" ")
                            print("-" * 60)
                        ...
                        save_credential(
                            create_credential(
                                username, sitename, account_name, password
                            )
                        )
                        print(" ")
                        print(
                            f"Credential Created: Site Name: {sitename} - Account Name: {account_name} - Password: {password}"
                        )
                        print(" ")
                    elif short_code == "sc":
                        print(" ")
                        if display_credentials(username):
                            ...
                            for credential in display_credentials(username):
                                print(
                                    f"Site Name: {credential.sitename} - Account Name: {credential.account_name} - Password: {credential.password}"
                                )
                            print(" ")
                        else:
                            print(" ")
                            print(
                                "There are no credentials saved with this account. Create a credential first"
                            )
                            print(" ")
                    elif short_code == "dc":
                        print(" ")
                        print("Enter the sitename of the account you want to delete")
                        sitename = input("Enter site's name- eg-Twitter ").strip()
                        print(delete_credential(sitename))
